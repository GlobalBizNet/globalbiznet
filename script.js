document.addEventListener('DOMContentLoaded', function() {
    ...
    // Reemplaza este número con tu número de WhatsApp Business completo, incluyendo el código de país.
    const phoneNumber = '573023741891'; 
    
    // Mensaje predeterminado opcional que el usuario enviará.
    const defaultMessage = 'Hola, estoy interesado en sus servicios y quisiera más información.';

    // --- Creación del Botón ---

    // Crear el elemento del botón (un enlace)
    const whatsappButton = document.createElement('a');
    
    // Configurar el enlace con el número y el mensaje
    whatsappButton.href = `https://wa.me/${phoneNumber}?text=${encodeURIComponent(defaultMessage)}`;
    whatsappButton.target = '_blank'; // Para que se abra en una nueva pestaña
    whatsappButton.id = 'whatsapp-button'; // ID para aplicarle estilos CSS
